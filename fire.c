...
int CanUsewep(edict_t *ent,int weapon)
{
	...
	// Bail early if the requested weapon's gitem_t pointer is NULL — happens
	// for Rogue / 3ZB2 weapons that aren't in Lithium's itemlist (e.g. Phalanx,
	// Ionripper, Trap). Bots will just pick another weapon.
	switch (weapon)
	{
		case WEAP_BLASTER:        if (!Fdi_BLASTER)        return qfalse; break;
		case WEAP_SHOTGUN:        if (!Fdi_SHOTGUN)        return qfalse; break;
		case WEAP_SUPERSHOTGUN:   if (!Fdi_SUPERSHOTGUN)   return qfalse; break;
		case WEAP_MACHINEGUN:     if (!Fdi_MACHINEGUN)     return qfalse; break;
		case WEAP_GRENADES:       if (!Fdi_GRENADES)       return qfalse; break;
		case WEAP_TRAP:           if (!Fdi_TRAP)           return qfalse; break;
		case WEAP_GRENADELAUNCHER:if (!Fdi_GRENADELAUNCHER)return qfalse; break;
		case WEAP_ROCKETLAUNCHER: if (!Fdi_ROCKETLAUNCHER) return qfalse; break;
		case WEAP_HYPERBLASTER:   if (!Fdi_HYPERBLASTER)   return qfalse; break;
		case WEAP_BOOMER:         if (!Fdi_BOOMER)         return qfalse; break;
		case WEAP_RAILGUN:        if (!Fdi_RAILGUN)        return qfalse; break;
		case WEAP_PHALANX:        if (!Fdi_PHALANX)        return qfalse; break;
		case WEAP_BFG:            if (!Fdi_BFG)            return qfalse; break;
		default: break;
	}
