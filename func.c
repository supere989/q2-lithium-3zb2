...
void Load_BotInfo()
{
	...
	//load info — prefer botpath (default "3zb2") so the merged mod can keep
	//the 3ZB2 asset folder while running under +set game lithium.
	{
		const char *cfgdir = (botpath && botpath->string && botpath->string[0])
		                   ? botpath->string : gamepath->string;
		sprintf(Buff,"%s/3ZBConfig.cfg",cfgdir);
	}
	fp = fopen(Buff,"rt");
	if(fp == NULL)
	{
...
void InitializeBot (edict_t *ent,int botindex )
{
	...
	ClientUserinfoChanged (ent, pinfo, sizeof(pinfo));
...
qboolean RemoveBot()
{
	...
	gi.cprintf (NULL, PRINT_HIGH, "Warning: Can't remove bot (edict already freed).\n");
	return qfalse;
}
