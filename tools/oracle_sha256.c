...
static void transform(hook_sha256_ctx_t *ctx, const uint8_t data[64])
{
	...
	for (i = 0; i < 64; ++i) {
		t1=h+E1(e)+CH(e,f,g)+constants[i]+m[i]; t2=E0(a)+MAJ(a,b,c);
...
void hook_sha256_update(hook_sha256_ctx_t *ctx, const void *input, size_t len)
{
...
void hook_sha256_final(hook_sha256_ctx_t *ctx, uint8_t hash[32])
{
	...
	for (i = 0; i < 4; ++i)
		for (uint32_t j = 0; j < 8; ++j) hash[i+j*4] = (uint8_t)(ctx->state[j] >> (24-i*8));
}

void hook_sha256_hex(const uint8_t hash[32], char out[65])
{
	static const char digits[] = "0123456789abcdef";
	for (int i = 0; i < 32; ++i) { out[i*2]=digits[hash[i]>>4]; out[i*2+1]=digits[hash[i]&15]; }
	out[64] = '\0';
}
